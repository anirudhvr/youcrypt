...
int yc_check_pass(const char *pass, char **reason)
{
	...
	if (passwdqc_params_parse(&params, &parse_reason, 0, pass)) {
        reason = &parse_reason;
        return 1;
    }
	...
	check_reason = passwdqc_check(&params.qc, pass, NULL, NULL);
	if (!check_reason) {
        return 0;
	} else {
        char *r = strdup(check_reason);
        *reason = r;
        return 1;
    }
}
