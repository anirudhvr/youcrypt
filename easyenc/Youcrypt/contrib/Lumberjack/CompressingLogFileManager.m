...
- (void)backgroundThread_CompressLogFile:(DDLogFileInfo *)logFile
{
	...
	do
	{
		...
		strm.avail_in = (unsigned int)inputDataSize;                      // as much as was read from file (plus leftovers).
		
		strm.next_out = (Bytef *)[outputData mutableBytes]; // Write data to output buffer
		strm.avail_out = (unsigned int)outputDataLength;                  // as much space as is available in the buffer.
